"""
backend/app/services/football_data_org_service.py

Purpose:
    Season import service for football-data.org fixtures using the unified
    match ingest service. Emits admin-job friendly counters and conflict
    previews.
...
Dependencies:
    ...
    - app.services.match_ingest_adapters.football_data_org_adapter
...
logger = logging.getLogger("quotico.football_data_org_service")
...
async def import_season(
    ...
    """Import all season matches for one league from football-data.org."""
    ...
    competition = str(ext_ids.get("football_data") or "").strip()
    if not competition:
        raise HTTPException(status_code=400, detail="League has no football_data external ID set")
    ...
    transformed, skipped_season_mismatch = await build_football_data_org_matches(
