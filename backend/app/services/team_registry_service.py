...
class TeamRegistry:
    ...
    def __init__(self):
        self.lookup_by_league: dict[tuple[str, int], ObjectId] = {}
    ...
    async def initialize(self) -> None:
        ...
        self.lookup_by_league.clear()
        self._global_candidates.clear()
        for team in teams:
            team_id = team["_id"]
            norm = team.get("normalized_name", "")
            league_id = team.get("league_id")
            if norm and isinstance(league_id, int):
                self.lookup_by_league[(norm, league_id)] = team_id
                self._global_candidates.setdefault(norm, set()).add(team_id)

            for alias in team.get("aliases", []):
                a_norm = alias.get("normalized", "")
                a_league_id = alias.get("league_id", league_id)
                if a_norm and isinstance(a_league_id, int):
                    self.lookup_by_league[(a_norm, a_league_id)] = team_id
        ...
        logger.info(
            ...
            len(self.lookup_by_league), len(self.lookup_global), len(self._global_ambiguous),
        )
    ...
    async def resolve(
        ...
        if not normalized:
            if not create_if_missing:
                return None
            return await self._auto_create(raw_name, normalized, league_id, reason="empty_normalization")

        # 1) Sport-scoped cache
        team_id = self.lookup_by_league.get((normalized, league_id))
        if team_id:
        ...
        team_id = await self._db_lookup(normalized, league_id)
        if team_id:
            return team_id
        ...
        return await self._auto_create(raw_name, normalized, league_id, reason=reason)
    ...
    async def resolve_by_external_id_or_name(
        ...
        resolved = await self.resolve(name, league_id, create_if_missing=create_if_missing)
        if not resolved:
            return None
    ...
    async def add_alias(
        ...
        *,
        ...
        alias_league_id = league_id if league_id is not None else team.get("league_id")
        if not isinstance(alias_league_id, int):
            raise ValueError("Alias league_id must be int")
        for existing in team.get("aliases", []):
            if not isinstance(existing, dict):
                continue
            if (
                str(existing.get("normalized") or "") == normalized
                and existing.get("league_id") == alias_league_id
            ):
                return False
    ...
    async def _db_lookup(self, normalized: str, league_id: int) -> ObjectId | None:
        """Check DB for team created by another worker since cache load."""
        doc = await _db.db.teams.find_one(
            {
                "$or": [
                    {"normalized_name": normalized, "league_id": league_id},
                    {"aliases": {"$elemMatch": {"normalized": normalized, "league_id": league_id}}},
                ]
            }
        )
        if not doc:
        ...
        team_id = doc["_id"]
        self.lookup_by_league[(normalized, league_id)] = team_id
        self._global_candidates.setdefault(normalized, set()).add(team_id)
        self._rebuild_global_lookup()
        self._stats["db_fallback_hits"] += 1
        logger.debug("DB fallback hit: '%s' (%s) -> %s", normalized, league_id, team_id)
        return team_id

    async def _auto_create(
        self, raw_name: str, normalized: str, league_id: int, *, reason: str
    ) -> ObjectId:
        """Create team doc with race-safe duplicate handling across workers."""
        now = utcnow()

        existing = self.lookup_by_league.get((normalized, league_id))
        if existing:
            return existing

        league = await LeagueRegistry.get().get_league(league_id)
        ...
        if created:
            logger.info(
                "Auto-created team: '%s' -> %s (league_id=%s, reason=%s, needs_review=True)",
                raw_name, team_id, league_id, reason,
            )
        return team_id
    ...
    def stats(self) -> dict:
        ...
        return {
            **self._stats,
            "sport_entries": len(self.lookup_by_league),
