"""
ESPN NBA Historical Data Backfiller for Quotico

Fetches historical NBA game results from ESPN's public scoreboard API
and pushes them to the Quotico backend via POST /api/historical/import.

NBA seasons are fetched in 3-month chunks (Oct-Dec, Jan-Mar, Apr-Jun) to
stay under ESPN's 1000-event-per-request limit. Follows the same patterns
as tools/scrapper.py and tools/nfl_history_backfiller.py.

Usage:
    python tools/nba_history_backfiller.py --api-url http://localhost:4201 --api-key dev123 --start-year 2024
    python tools/nba_history_backfiller.py --start-year 2015 --end-year 2024 --dry-run
    python tools/nba_history_backfiller.py --start-year 2002 --end-year 2024 --api-key YOUR_KEY
"""
...
log = logging.getLogger("nba_backfiller")
...
SLEEP_BETWEEN_CHUNKS = 1  # seconds between ESPN requests

# NBA season spans Oct–Jun. Split into 3 chunks to stay under ESPN's 1000-event cap.
# Each chunk is (start_month, start_day, end_month, end_day, year_offset_start, year_offset_end)
# year_offset: 0 = season year, 1 = season year + 1
SEASON_CHUNKS = [
    ("Oct-Dec", 10, 1, 12, 31, 0, 0),
    ("Jan-Mar",  1, 1,  3, 31, 1, 1),
    ("Apr-Jun",  4, 1,  6, 30, 1, 1),
]
...
def fetch_chunk(start_date: str, end_date: str) -> list[dict]:
    """Fetch all events for a date range. Returns list of ESPN event dicts."""
...
def _is_all_star(event: dict) -> bool:
    """Check if an event is an All-Star game."""
    name = event.get("name", "").lower()
    short_name = event.get("shortName", "").lower()
    return "all-star" in name or "all star" in name or "all-star" in short_name


def event_to_record(event: dict, season_year: int) -> dict | None:
    """Convert an ESPN event to a HistoricalMatch record."""
    # Skip All-Star games
    if _is_all_star(event):
        return None
    ...
    # NBA has no ties (overtime until decided)
...
def main():
    parser = argparse.ArgumentParser(
        description="Import historical NBA data from ESPN into Quotico"
    )
    ...
    parser.add_argument("--start-year", type=int, default=DEFAULT_START_YEAR,
                        help=f"First NBA season year (default: {DEFAULT_START_YEAR})")
    parser.add_argument("--end-year", type=int, default=None,
                        help="Last NBA season year (default: current year - 1)")
    ...
    log.info("=" * 60)
    log.info("NBA Historical Backfill — %s", SPORT_KEY)
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        for chunk_label, sm, sd, em, ed, so, eo in SEASON_CHUNKS:
            start = f"{year + so}{sm:02d}{sd:02d}"
            end = f"{year + eo}{em:02d}{ed:02d}"

            try:
                events = fetch_chunk(start, end)
            except Exception as e:
                log.error("  %s %s: fetch failed: %s", label, chunk_label, e)
                total_errors += 1
                continue
            ...
            if chunk_records:
                log.info("  %s %s: %d games", label, chunk_label, len(chunk_records))

            season_records.extend(chunk_records)
            total_chunks += 1
